...
- 支持 GLB 基础材质：`baseColorFactor`、`baseColorTexture`、`metallicFactor`、`roughnessFactor`、`metallicRoughnessTexture`、`normalTexture`、`emissiveFactor`、透明度 alpha
- 支持 GLB 贴图读取，可读取内嵌 bufferView 图片或外部图片路径
- 支持 GLB `alphaMode: "BLEND"` 自动识别为透明介质（Dielectric），常用于导出工具的玻璃近似
- 支持 GLB KHR 扩展：`KHR_materials_transmission`（透射）、`KHR_materials_ior`（折射率）、`KHR_materials_volume`（衰减元数据）
- GLB 材质路由优先级：emissiveFactor > KHR transmission > alphaMode BLEND > metallic > lambertian
