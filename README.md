...
CMake 测试包含 C++ 回归测试和 deterministic golden image 回归；golden 覆盖基础球、镜面/玻璃/水、Khronos GLB PBR/透明材质、OBJ MTL 导入、glass_emissive、bunny/mark OBJ、no-normal OBJ、扩展 MTL、studio preset 以及 Khronos CompareRoughness / TransmissionRoughnessTest。更新 golden 图片时先确认渲染变化符合预期，再运行：
...
- 支持 GLB `alphaMode: "BLEND"` 自动识别为透明介质（Dielectric）；`alphaMode: "MASK"` + `alphaCutoff` 会真正生效：命中点 alpha 低于 cutoff 时丢弃命中并让光线穿过
