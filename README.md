...
- 支持 GLB `alphaMode: "BLEND"` 自动识别为透明介质（Dielectric），支持记录 `alphaMode: "MASK"` / `alphaCutoff`
- 支持 GLB KHR 扩展：`KHR_materials_transmission`（透射与 transmissionTexture）、`KHR_materials_ior`（折射率）、`KHR_materials_volume`（透明介质衰减、thicknessFactor、thicknessTexture）
