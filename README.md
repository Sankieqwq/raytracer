...
- 支持 OBJ `.mtl` 的 `Kd` / `map_Kd`、`Ke` / `map_Ke`、`d` / `Tr`、`Ni`、`Ks`、`Ns`，可绑定基础颜色、贴图、自发光、透明度、折射率和高光粗糙度近似
...
- 支持 GLB KHR 扩展：`KHR_materials_transmission`（透射）、`KHR_materials_ior`（折射率）、`KHR_materials_volume`（透明介质衰减）
